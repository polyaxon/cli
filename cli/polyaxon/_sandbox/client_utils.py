...
def normalize_command(command: Iterable[str]) -> List[str]:
    if isinstance(command, str):
        raise TypeError("command must be an iterable of strings, not a string")
    try:
        value = list(command)
    except TypeError as e:
        raise TypeError("command must be an iterable of strings") from e
    if not value:
...
def normalize_env(env: Optional[Mapping[str, Optional[str]]]) -> Optional[Dict]:
    if env is None:
        return None
    if not isinstance(env, MappingABC):
