...
class TestSshSetup(BaseTestCase):
    def setUp(self):
    ...
    def test_ensure_local_keypair_generates_default_key(self):
        identity_file = self.tmp_path / ".ssh" / "polyaxon_sandbox_ed25519"
    ...
    def test_ensure_local_keypair_reuses_existing_key(self):
        identity_file = self.tmp_path / "id_ed25519"
    ...
    def test_ensure_local_keypair_rejects_missing_explicit_key(self):
        identity_file = self.tmp_path / "missing"

        with self.assertRaisesRegex(PolyaxonClientException, "does not exist"):
            setup.ensure_local_keypair(str(identity_file))

    def test_get_public_key_reads_pub_file(self):
        identity_file = self.tmp_path / "id_ed25519"
    ...
    def test_get_public_key_derives_from_private_key_when_pub_file_missing(self):
        identity_file = self.tmp_path / "id_ed25519"
        identity_file.write_text("private", encoding="utf-8")
    ...
    def test_build_remote_setup_command_is_idempotent(self):
    ...
    def test_prepare_ssh_access_pushes_public_key_through_sandbox(self):
        identity_file = self.tmp_path / "id_ed25519"
        identity_file.write_text("private", encoding="utf-8")
    ...
    def test_prepare_ssh_access_raises_on_remote_failure(self):
        identity_file = self.tmp_path / "id_ed25519"
        identity_file.write_text("private", encoding="utf-8")
        Path("{}{}".format(identity_file, ".pub")).write_text(
        ...
        with self.assertRaisesRegex(PolyaxonClientException, "boom"):
            setup.prepare_ssh_access(client, identity_file=str(identity_file))

    def test_prepare_ssh_access_rejects_missing_host_public_key(self):
        identity_file = self.tmp_path / "id_ed25519"
        identity_file.write_text("private", encoding="utf-8")
        Path("{}{}".format(identity_file, ".pub")).write_text(
        ...
        with self.assertRaisesRegex(PolyaxonClientException, "host public key"):
            setup.prepare_ssh_access(client, identity_file=str(identity_file))

    def test_resolve_known_hosts_file_defaults_to_polyaxon_managed_path(self):
        assert (
            setup.resolve_known_hosts_file()
            == Path(POLYAXON_KNOWN_HOSTS_PATH).expanduser()
        )

    def test_write_known_hosts_entry_creates_file(self):
        known_hosts = self.tmp_path / ".polyaxon" / "known_hosts"
        ...
        assert (
            known_hosts.read_text(encoding="utf-8") == "polyaxon-run ssh-ed25519 HOST\n"
        )
        assert stat.S_IMODE(known_hosts.stat().st_mode) == 0o600
        assert stat.S_IMODE(known_hosts.parent.stat().st_mode) == 0o700

    def test_write_known_hosts_entry_replaces_alias_and_preserves_others(self):
        known_hosts = self.tmp_path / "known_hosts"
    ...
    def test_write_known_hosts_entry_rejects_invalid_host_key(self):
        with self.assertRaises(PolyaxonClientException):
            setup.write_known_hosts_entry(
                path=self.tmp_path / "known_hosts",
                alias="polyaxon-run",
