...
class TestClientMixin(BaseTestCase, IsolatedAsyncioTestCase):
    def setUp(self):
    ...
    def test_reset_client_in_cluster_is_noop(self):
        with patch.object(settings.CLIENT_CONFIG, "in_cluster", True):
    ...
    async def test_areset_client_raises_on_sync_mixin(self):
        mixin = DummyClient()

        with self.assertRaises(PolyaxonClientException):
            await mixin.areset_client()

    async def test_areset_client_closes_previous_after_replacement(self):
        events = []
    ...
    async def test_areset_client_in_cluster_is_noop(self):
        with patch.object(settings.CLIENT_CONFIG, "in_cluster", True):
    ...
    async def test_flush_on_exit_default_is_noop(self):
        mixin = AsyncDummyClient()

        assert await mixin._flush_on_exit() is None

    async def test_owned_async_client_closes_on_context_exit(self):
        client = SDKClientMock(is_async=True)
        mixin = AsyncDummyClient()
        mixin._client = client
        mixin._owns_client = True
    ...
    async def test_injected_async_client_does_not_close_on_context_exit(self):
        client = SDKClientMock(is_async=True)
        mixin = AsyncDummyClient()
        mixin._set_client(client)
    ...
    async def test_async_context_manager_raises_on_sync_mixin(self):
        mixin = DummyClient()
    ...
    async def test_async_context_manager_flushes_before_close(self):
        events = []
    ...
    async def test_async_context_manager_closes_when_flush_raises(self):
