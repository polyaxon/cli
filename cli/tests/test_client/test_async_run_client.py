...
@pytest.mark.asyncio
class TestAsyncRunClient(BaseTestCase, IsolatedAsyncioTestCase):
    ...
    def make_client(self, sdk_client):
    ...
    def get_logged_lineage_artifact(self, sdk_client, index=0):
        body = sdk_client.runs_v1.create_run_artifacts_lineage.call_args_list[index][1][
            "body"
        ]
        return body.artifacts[0]

    def test_async_run_client_uses_async_store(self):
        client = self.make_client(AsyncPolyaxonClientMock())

        assert isinstance(client.store, AsyncPolyaxonStore)

    def test_async_run_client_public_export(self):
        from polyaxon.client import AsyncRunClient as Exported

        assert Exported is AsyncRunClient

    def test_async_run_client_rejects_sync_client(self):
        with self.assertRaises(PolyaxonClientException):
            AsyncRunClient(
                owner=OWNER, project=PROJECT, client=SyncPolyaxonClientMock()
            )

    def test_async_run_client_method_surface_is_async(self):
    ...
    def test_async_run_client_local_helpers_stay_shared(self):
        helpers = [
            "_apply_created_run",
    ...
    async def test_refresh_data_loads_nested_data_sequentially(self):
        sdk_client = AsyncPolyaxonClientMock()
        events = []

        async def get_run(owner, project, run_uuid):
            events.append("run")
            return self.make_run(status=V1Statuses.RUNNING)
    ...
    async def test_update_awaits_api_without_async_req_and_mutates_state(self):
        sdk_client = AsyncPolyaxonClientMock()
        response = self.make_run(description="updated")
    ...
    async def test_metadata_methods_await_api_without_overwriting_local_state(self):
        sdk_client = AsyncPolyaxonClientMock()
        sdk_client.runs_v1.patch_run = AsyncMock(
            return_value=self.make_run(name="remote")
        )
        client = self.make_client(sdk_client)
    ...
    async def test_create_awaits_api_without_async_req_and_mutates_state(self):
        sdk_client = AsyncPolyaxonClientMock()
        created = self.make_run(name="created")
    ...
    async def test_status_methods_await_api_without_async_req(self):
        ...
        condition = sdk_client.runs_v1.create_run_status.call_args[1]["body"][
            "condition"
        ]
        assert condition.type == V1Statuses.RUNNING
        assert "async_req" not in sdk_client.runs_v1.create_run_status.call_args[1]

    async def test_wait_for_condition_updates_status(self):
        sdk_client = AsyncPolyaxonClientMock()
    ...
    async def test_get_logs_awaits_refresh_and_api(self):
        sdk_client = AsyncPolyaxonClientMock()
        response = mock.Mock()
        sdk_client.runs_v1.get_run = AsyncMock(return_value=self.make_run())
    ...
    async def test_inspect_awaits_refresh_and_api(self):
        sdk_client = AsyncPolyaxonClientMock()
        response = {"pods": {}}
        sdk_client.runs_v1.get_run = AsyncMock(
            return_value=self.make_run(status=V1Statuses.RUNNING)
        )
    ...
    async def test_events_metrics_and_lineage_methods_await_api(self):
        sdk_client = AsyncPolyaxonClientMock()
        ...
        sdk_client.runs_v1.get_runs_artifacts_lineage = AsyncMock(
            return_value=mock.Mock()
        )
        client = self.make_client(sdk_client)
        client._run_data = self.make_run()
    ...
    async def test_artifact_stream_methods_await_api_without_async_req(self):
        sdk_client = AsyncPolyaxonClientMock()
        ...
        assert await client.get_artifact(
            "outputs/model.pkl", stream=False, force=True
        ) == (b"payload")
    ...
    async def test_get_artifact_awaits_refresh_when_settings_missing(self):
        sdk_client = AsyncPolyaxonClientMock()
        sdk_client.runs_v1.get_run = AsyncMock(return_value=self.make_run())
        sdk_client.runs_v1.get_run_artifact = AsyncMock(return_value=b"payload")
        client = self.make_client(sdk_client)
    ...
    async def test_download_artifact_methods_call_store_download_file(self):
        sdk_client = AsyncPolyaxonClientMock()
        sdk_client.config = mock.Mock(host="http://api", no_op=None, is_offline=None)
        client = self.make_client(sdk_client)
        client._run_data = self.make_run()
        client._store = mock.Mock()
        client._store.download_file = AsyncMock(
            side_effect=["/tmp/file", "/tmp/archive"]
        )
    ...
    async def test_download_artifact_for_metric_lineage_downloads_file(self):
        sdk_client = AsyncPolyaxonClientMock()
        client = self.make_client(sdk_client)
        client._run_data = self.make_run()
    ...
    async def test_download_artifact_for_model_lineage_downloads_artifacts(self):
        sdk_client = AsyncPolyaxonClientMock()
        client = self.make_client(sdk_client)
        client._run_data = self.make_run()
        client.download_artifact = AsyncMock(return_value="file")
    ...
    async def test_download_artifact_for_directory_lineage_downloads_artifacts(self):
        sdk_client = AsyncPolyaxonClientMock()
        client = self.make_client(sdk_client)
        client._run_data = self.make_run()
        client.download_artifact = AsyncMock(return_value="file")
    ...
    async def test_download_artifact_for_lineage_downloads_event_package(self):
        sdk_client = AsyncPolyaxonClientMock()
        sdk_client.config = mock.Mock(host="http://api", no_op=None, is_offline=None)
        client = self.make_client(sdk_client)
        client._run_data = self.make_run()
    ...
    async def test_run_action_methods_await_api_without_async_req(self):
        sdk_client = AsyncPolyaxonClientMock()
        for sdk_method_name in [
            "stop_run",
            "skip_run",
            "approve_run",
            "invalidate_run",
    ...
    async def test_list_delete_namespace_and_transfer_await_api(self):
        sdk_client = AsyncPolyaxonClientMock()
        ...
        sdk_client.organizations_v1.get_organization_runs = AsyncMock(
            return_value=all_runs
        )
    ...
    async def test_log_artifact_lineage_and_run_edges_await_api(self):
        sdk_client = AsyncPolyaxonClientMock()
        ...
        assert (
            await client.log_artifact_lineage(
                V1RunArtifact.model_construct(name="model")
            )
            is None
        )
        ...
        assert (
            "async_req"
            not in sdk_client.runs_v1.create_run_artifacts_lineage.call_args[1]
        )
    ...
    async def test_log_code_ref_detects_code_ref_in_thread(self):
        sdk_client = AsyncPolyaxonClientMock()
        sdk_client.runs_v1.create_run_artifacts_lineage = AsyncMock(return_value=None)
        client = self.make_client(sdk_client)
        ...
        artifact = self.get_logged_lineage_artifact(sdk_client)
    ...
    async def test_log_data_ref_hashes_content_and_awaits_lineage_api(self):
        sdk_client = AsyncPolyaxonClientMock()
        sdk_client.runs_v1.create_run_artifacts_lineage = AsyncMock(return_value=None)
        client = self.make_client(sdk_client)
    ...
    async def test_log_artifact_ref_hashes_existing_file(self):
        asset = self.tmp_path / "result.json"
    ...
    async def test_log_model_ref_updates_meta_and_awaits_lineage_api(self):
        sdk_client = AsyncPolyaxonClientMock()
        sdk_client.runs_v1.patch_run = AsyncMock(return_value=self.make_run())
        sdk_client.runs_v1.create_run_artifacts_lineage = AsyncMock(return_value=None)
        client = self.make_client(sdk_client)
    ...
    async def test_log_file_and_dir_refs_hash_paths_and_await_lineage_api(self):
        file_path = self.tmp_path / "file.txt"
        file_path.write_text("payload")
        dir_path = self.tmp_path / "outputs"
        dir_path.mkdir()
        ...
        file_artifact = self.get_logged_lineage_artifact(sdk_client, 0)
        dir_artifact = self.get_logged_lineage_artifact(sdk_client, 1)
    ...
    async def test_log_tensorboard_ref_logs_once_and_sets_meta(self):
        sdk_client = AsyncPolyaxonClientMock()
        sdk_client.runs_v1.patch_run = AsyncMock(return_value=self.make_run())
        sdk_client.runs_v1.create_run_artifacts_lineage = AsyncMock(return_value=None)
        client = self.make_client(sdk_client)
    ...
    async def test_promote_methods_use_async_project_client_with_injected_client(self):
        sdk_client = AsyncPolyaxonClientMock()
    ...
    async def test_sync_only_methods_raise(self):
        for method, args in [
    ...
    async def test_load_offline_run_is_sync_only(self):
        with self.assertRaises(PolyaxonClientException):
    ...
    async def test_resume_awaits_resume_run_with_built_body(self):
        sdk_client = AsyncPolyaxonClientMock()
        response = self.make_run(name="resumed")
    ...
    async def test_log_succeeded_awaits_create_run_status_via_log_status(self):
        ...
        sdk_client.runs_v1.create_run_status = AsyncMock(return_value=None)
        client = self.make_client(sdk_client)
        client._run_data = self.make_run(status=V1Statuses.RUNNING)
    ...
    async def test_log_end_status_short_circuits_when_already_done(self):
        sdk_client = AsyncPolyaxonClientMock()
        sdk_client.runs_v1.create_run_status = AsyncMock()
        client = self.make_client(sdk_client)
        client._run_data = self.make_run(status=V1Statuses.SUCCEEDED)
