...
class TestSDKTransport(BaseTestCase, IsolatedAsyncioTestCase):
    def setUp(self):
    ...
    async def get_async_request_timeout(self, configuration, request_timeout=None):
    ...
    def get_sync_request_timeout(self, configuration, request_timeout=None):
    ...
    def test_sync_rest_uses_configured_timeout(self):
        ...
        timeout = self.get_sync_request_timeout(configuration)
    ...
    def test_sync_rest_request_timeout_overrides_config(self):
        ...
        timeout = self.get_sync_request_timeout(configuration, request_timeout=3)
    ...
    def test_sync_rest_uses_no_timeout_without_configured_timeout(self):
        ...
        assert self.get_sync_request_timeout(configuration) is None

    def test_sync_rest_keeps_connect_and_read_timeout_support(self):
        configuration = Configuration(timeout=None)

        timeout = self.get_sync_request_timeout(configuration, request_timeout=(2, 4))
    ...
    async def test_async_rest_uses_configured_timeout(self):
        configuration = ClientConfig(host="localhost", timeout=17).async_sdk_config

        assert configuration.timeout == 17
        assert await self.get_async_request_timeout(configuration) == 17

    async def test_async_rest_request_timeout_overrides_config(self):
        configuration = ClientConfig(host="localhost", timeout=17).async_sdk_config

        assert (
            await self.get_async_request_timeout(configuration, request_timeout=3) == 3
        )

    async def test_async_rest_uses_no_timeout_without_configured_timeout(self):
        configuration = Configuration(timeout=None)

        assert await self.get_async_request_timeout(configuration) is None

    def test_sync_api_error_preserves_exception_and_adds_request_context(self):
    ...
    async def test_async_api_error_preserves_exception_and_adds_request_context(self):
        for status in (403, 404, 500, 599):
            with self.subTest(status=status):
                client = AsyncApiClient(ClientConfig(host="localhost").async_sdk_config)
                ...
                assert error.headers == {"X-Request-ID": "request-id"}
                assert (
                    error.request_context == "POST /api/v1/my%20org/my%2Fproject/runs"
                )
    ...
    def test_api_exception_without_request_context_keeps_existing_format(self):
    ...
    def test_cli_api_error_displays_request_context_once(self, stdout):
        for status in (403, 404, 500, 599):
            with self.subTest(status=status):
                stdout.seek(0)
                stdout.truncate(0)
                ...
                assert output.count("HTTP request: GET /api/v1/test") == 1
    ...
    @patch("sys.stdout", new_callable=StringIO)
    def test_cli_api_error_without_request_context_keeps_existing_output(self, stdout):
        for status in (403, 404, 500, 599):
            with self.subTest(status=status):
                stdout.seek(0)
                stdout.truncate(0)
